...
// Define initialState first so we can reference its objects in perfectActions
    ...
    ailments: [
      {
        ...
        normality: "No man, my ankle isn't usually broken, believe it or not.",
...
// Now create the scenario with perfectActions referencing the initialState objects
const mvpScenario: CreateScenarioSchema = {
  ...
  perfectActions: [
    { verb: 'look' as const, object: initialState.patient, modifiers: [] },
    {
      verb: 'palpate' as const,
      object: initialState.patient.bodyParts.find(
        (part) => part.partName === 'leftFoot',
      )!,
      modifiers: [],
    },
  ] as const,
